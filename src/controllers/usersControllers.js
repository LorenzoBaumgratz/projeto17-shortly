...
export async function ranking(req,res){
    try{
        const result=await db.query(`select sum("visitCount") as total,count("urls.id") as qnt,urls."visitCount",urls.id as "idDaUrl",users.name,users.id as "idDoUsuario",sessions.id from urls left join sessions on urls."sessionId"=sessions.id left join users on sessions."userId"=users.id group by users.id order by total limit 10;`)
